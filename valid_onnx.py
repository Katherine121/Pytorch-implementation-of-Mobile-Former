...
if __name__ == "__main__":
    onnx_model_path = "./acc/mobile_former_151.onnx"
    # onnx测试
    ...
    with torch.no_grad():
        ...
        inputs = {onnx_session.get_inputs()[0].name: to_numpy(image_tensor)}
        outs = onnx_session.run(None, inputs)[0]

        preds = outs.argmax(axis=1)[0]
    print(outs)
    print(preds)
...
#     # cifar_val += cifar_val_aug
#
#     loader_val = DataLoader(cifar_val, batch_size=1, shuffle=True, pin_memory=True)
#     print(len(cifar_val))
#
...
#
#             # compute ONNX Runtime output prediction
#             inputs = {onnx_session.get_inputs()[0].name: to_numpy(x)}
#             outs = onnx_session.run(None, inputs)[0]
#
#             preds = outs.argmax(axis=1)[0]
#             num_correct = (preds == y)
